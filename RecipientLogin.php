...
if (isset($_POST['login'])) {
    ...
    $sql = "SELECT password FROM recipient WHERE mobile='$mobileNumber';";
    $result = mysqli_query($connection, $sql);
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        $recipientPassword = $row['password'];
        if (password_verify($password, $recipientPassword)) {
            session_start();
            $sql = "SELECT id FROM recipient WHERE mobile='$mobileNumber';";
            $result = mysqli_query($connection, $sql);
            $row = mysqli_fetch_assoc($result);
            $_SESSION["id"] = $row['id'];
            $_SESSION["role"] = 2;
            header("Location:RecipientHomepage.php");
        } else {
            echo "<script>
        ...
        echo "<script>
		 alert('Incorrect mobile number, enter your mobile number again');
		 </script>";
    }
}
?>
...
<html>
    ...
    <head>
        <title> A Meal For You - Recipient Login </title>
        <meta charset="UTF-8">
    ...
    <body>
        ...
        <div class="signup_split">
            <div class="LeftSide">
                <section class="copy">
                    <div class="slide">
                        <h1><strong>Recipient Log In</strong></h1>
                        <p></p>
                    </div>
                </section>
            </div>
            ...
            <div class="RightSide">
                ...
                <form class="RegisterForm" action="RecipientLogin.php" method ="POST">
                    <section class="copy">
                        <h2>Log In</h2>
                        <div class="Login">
                            <p>Don't have an account? <a href="RecipientSignup.php"><strong>Sign Up</strong></a></p>
                        </div>
                    </section>
                    <div id="checkphone">
                    ...
                    <div class="input-container-phone">
                        ...
                        <input id="mobile" type="tel" name="mobile" placeholder="05XXXXXXX" pattern="[0-9]{3}[0-9]{3}[0-9]{4}" value="">
                    </div>
